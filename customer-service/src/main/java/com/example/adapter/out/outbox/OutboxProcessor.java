    ...
    public void processOutbox() {
        ...
        for (var message : newBatch) {
            var producerRecord = new ProducerRecord<>(
                    customerMonitoringTopic,
                    message.getAggregateId(),
                    message.getPayload()
            );
            var headers = producerRecord.headers();
            headers.add("messageId", message.getId().toString().getBytes(StandardCharsets.UTF_8));
            headers.add("type", message.getEventType().getBytes(StandardCharsets.UTF_8));
